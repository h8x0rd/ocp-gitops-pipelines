...
const BROKERS = (process.env.BROKERS || 'broker-a,broker-b,broker-c')
  ...
  .map((s) => s.trim())
  .filter(Boolean);
const DEMO_ENV = process.env.DEMO_ENV || 'unknown';
const DEMO_RELEASE_ID = process.env.DEMO_RELEASE_ID || 'untracked';
const PROMOTED_FROM = process.env.PROMOTED_FROM || 'unknown';
const PROMOTED_AT = process.env.PROMOTED_AT || 'unknown';
const PROMOTED_BY = process.env.PROMOTED_BY || 'unknown';
